...
#### 확룰분포함수 (probability distribution functionh)
...
확률변수 $X$는 자신만의 새로운 확률함수와 확률분포함수를 정의하게 되는데 그 과정을 엄미하게 살펴보자.
먼저 새로 정의할 함
사실 어떤식으로 이것이 가능한지는 아마 벌써 예상될지도 모르겠다.
확률변수 $X$의 확률함수 $p_X$는 다음과 같이 정의된다:

$$p_X(a) = p[X=a] = \sum_{a \in [X=a]}f(a).$$


$$f_X(a) = p[X=a] = \sum_{a \in [X=a]}f(a).$$
