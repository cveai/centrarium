...
시점이 $t$일 때 agent가 할 수 있는 선택을 action이라고 한다.
Agent가 이 게임에서 취할 수 있는 모든 가능한 action을 다 모아놓은 집합을 $\mathscr{A}$라고 표시하고, 
시점 $t$에 취하는 action을 $A_t$라고 표시하자. 그러면 자연스럽게 $A_t \in \mathscr{A}$임을 알 수 있다.
반면, 각 시점에서 environment는 두가지 반응으로 agent의 action에 대응하는데 그것이 각각 state와 reward이다.
Action의 경우와 마찬가지로 시점 $t$dp environment가 내놓게 되는 state를 $S_t$, reward를 $R_t$로 표시한다.
