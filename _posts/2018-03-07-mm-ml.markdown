...
1. Averaging over $\theta$ gieve the data: $E(L(\theta, f(D)) \vert D =:\rho(\pi, f(D))$ (<span style="color:blue">Bayesian expected loss</span>) 
2. Averaging over the data gieve $\theta$: $E(L(\theta, f(D)) \vert \theta =: R(\theta, f)$ (<span style="color:red">(Frequentist) risk</span>) 


<figure>
<img src="/assets/pics/mm-ml/bayes-risk.png" alt="Bayes Risk" style="width: 80%; height: 80%">
<figcaption>
</figcaption>
</figure>
...
A way to *reject* certain estimators.

**Recall** the frequentist risk $R(\theta, \delta) = E(L(\theta, \delta(D) \vert \theta)$ for the decition rule $\delta$

**Example** $X \sim N(\theta, 1), \theta \in \mathbb{R}, D=X, L(\theta, \hat{\theta})$  
"Natual estimator": $\delta_1(D) = X$, ${\rm MSE}(\delta) = 1 = R(\theta, \delta_1)  
"Silly" estimator: $\delta_0(D) = X$, ${\rm MSE}(\delta) = 1 = R(\theta, \delta_1)  
