...
$X \sim N(\theta, \sigma^2)$, i.e., target distribution fanily $(N(\theta, \sigma^2))_{\theta \in \Theta}$
...
For MLE, we wanna maximize, over $\theta$, the following:

$p(D \vert \theta) = p(x_1, \ldots, x_b \vert \theta) = \prod_{i=1}^n p(x_i \vert \theta) = (\frac{1}{\sqrt{2\pi\sigma^2}})^n\exp\left(-\frac{1}{2\sigma}\sum_{i=1}^n(x - \theta)^2 \right)$ 
