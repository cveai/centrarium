...
그 다음 스텝의 업데이트를 위한 loss 는

$$L_{t+1}(\theta_{t+1}) = (r_t + \gamma \max_{a'}Q_{\theta_{t+1}}(s_{t+1},a') - Q_{\theta_{t+1}}(s_t, a_t))^2$$ 

으로 temporal difference 계산을 위한 타겟을 주는 함수가 $Q_{\theta_t}$ 에서 $Q_{\theta_{t+1}}$ 로 바뀌어있는 상태이다.

진행하면 ${\rm Loss}(\theta_0) > {\rm Loss}(\theta_1) > {\rm Loss}(\theta_2) ...$ 이런 식으로 
