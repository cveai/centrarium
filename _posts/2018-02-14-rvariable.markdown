...
확률론이 측도론을 통하여 현대적인 엄밀한 학문으로 확립된 것은 Kolmogorov 라는 러시아 수학자의 업적으로 알려져 있다.
...
확률변수 $X$가 의미하는 바를 알아챘는가? 그렇다. $X(\square)$는 $\square$가 나타내는 주사위 눈의 개수를 나타낸다. 
...
3. 이런 문제에 더해 한층 혼란을 가중시키는 요소인 조건부 확률에 대해서는 다음 기회에 살펴보기로 한다.
