...
In sum, we target $\theta_{\rm MLE}$ from the target distributoins $(N(\theta, \sigma^2))_{\theta \in \Theta}$
and the sample mean $\frac{1}{n}\sum_{i=1}^n X_i$ is an effective estimator of it.
