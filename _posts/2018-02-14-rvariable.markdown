...
측도론을 이용한 통합적인 접근법을 이해하기 위해서는 Mahtematical Monk의 동영상 강의를 강력히 추천한다: [3].
확률론이 측도론을 통하여 현대적인 엄밀한 학문으로 확립된 것은 Kolmogorov 라는 러시아 수학자의 업적으로 알려져있다.
...
#### 확룰분포함수 (probability distribution function)
...
확률함수의 두번째 성질인 덧셈 원리를 잘 이해하면 각 표본에 대한 함숫값을 정해주는 것만으로 충분하다는 사실을 알아낼 수 있다.
이런 관찰을 바탕으로 **확률분포함수**(probability distribution function)라는 것을 도입할 수 있다.
...
일반적으로 다음 두가지 성질을 만족하는 함수를 확률분포함수라고 한다.
...
사실, "확률분포" 라고 하면 많은 사람들이 이런 그래프를 떠올리는 듯 하다.
필자의 생각에도 확률분포라는 개념은 확률분포함수의 그래프로 이해하는 것이 직관적인 것 같다.
...
확률함수 혹은 확률분포함수가 주어진 후의 논리 전개 및 관련 계산은 많은 경우 확률변수라는 개념을 통해 이루어진다. 확률변수의 정의는 간단한다. 
...
확률변수 $X$가 의미하는 바를 알아챘는가? 그렇다. $X(\square)$는 $\square$가 나타내는 주사위 눈의 갯수를 나타낸다. 
...
그러면 $[Y = 1] = \\{a \in \Omega : Y(a) = 1\\} = \\{ ⚀, ⚂, ⚄ \\}$는 확률변수 $Y$에 대한 함수값이 1인 표본들로 구성돤 사건이다.  
...
확률함수 $p$ 혹은 그에 대응되는 확률분포함수 $f$에 기반한 확률변수 $X$를 생각하자.
확률함수와 확률분포함수가 본질적으로 같은 정보의 두가지 다른 표현이라는 것을 상기하면
두 함수가 다음의 관계를 만족 함을 알 수 있다:
...
이 됨을 알 수 있다. 확률변수 $X$를 통한 두 확률함수 $p$와 $p_X$ 사이의 이 관계를 기억하도록 하자.
주사위 실험과 관련하여 정의했었던 확룰함수 $X$와 $Y$를 이용하여 이 내용들을 확인해보는 것이 $p_X$와 $p_Y$를 이해하는데 도움이 될 것 같다. 
...
즉, 기댓값을 계산하는 과정에서 나타나는 확률함수는 본질적으로 $p$뿐이다. 
...
확률변수를 중심으로 확률 개념을 다루는데 있어 발생하는 노테이션 문제 몇가지를 짚어보자.

1. 조금 전에 본 $f_{XY}$, $f_{X}$, $f_{Y}$ 라는 표현은 혼란을 일으킨다. 왜냐하면 $\color{red}{f}_{XY}$라는 함수는 
$\color{blue}{f}$와 $g$라는 함수에 의존해서 정의되는데 표현의 문제로 봤을 때 두 "$f$"는 서로 다른 $f$이기 때문이다.
또한 $f_X$와 $f$에서 "$f$"는 다른 의미로 쓰이지만 $f_x = f$이라는 사실이 혼란을 가중시킨다.
따라서 이런 종류의 표현을 만났을 때는 "$f$"보다는 $XY$, $X$, $Y$라는 부분에 집중하여 
...
대치된 표현들이 나타나면 확률변수 $X, Y$가 기반하는 확률분포함수 $f, g$를 각각 찾아내어 $\mathrm{Pr}[X=x, Y=y] = \sum_{X(a) = x, \ Y(b) = y} f(a)g(b)$로
인식하는 것이 혼란을 줄이는 접근법이다. 상황에 따라서는 기반 확률분포함수 $, g$가 불투명하고 그 대신 결합확뷸뷴포함수 $f_{XY}$가 주어진 경우 부변 분포함수 $f_X, f_Y$를
