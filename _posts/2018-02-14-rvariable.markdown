...
앞서 논의한 결합 분포와 관련하여 분리 분포의 개념을 정의할 수 있다.
...
1. 조금 전에 본 $f_{XY]$라는 표현은 혼란을 일으킨다. 왜냐하면 $\color{red}{f}_{XY}$라는 함수는 
$\color{blue}{f}$와 $g$라는 함수에 의존해서 정의되는데 표현의 문제로 봤을 때 두 "$f$"는 서로 다른 $f$이기 떄문이다.
