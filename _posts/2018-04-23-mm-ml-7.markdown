...
Setup: $X_1, \ldots, X_n \sim {\rm Bern}(\theta)$ with prior $\theta \sim {\rm Beta}(a,b)$.

For generaic $X \sim {\rm Bern}(\theta)$, we have

$$p(1\vert \theta) = P(X = 1 \vert \theta) = \theta$$

where

$$p(\theta) = \frac{\theta^{a-1}(1-\theta)^{b-1}}{B(a,b)} \propto \theta^{a-1}(1-\theta)^{b-1}$$

From the setup we generate data $D = (x_1, \ldots, x_n)$. Then

$P(X = 1\vert \theta) = \theta = \theta^{I(X=1)}(1-\theta)^{I(X=0)}$



$$\begin{aligned}
p(\theta\vert D) & \propto_\theta p(\theta\vert \theta) p(\theta) \\
                 & = p(\theta)\prod_{i=1}p(x_i\vert \theta)$ \\
                 ...
                 & \propto {\rm Beta}(\theta\vert a+n_1, b+n_0)
\end{aligned}$$

Thus, $p(\theta\vert D) = {\rm Beta}(\theta\vert a+n_1, b+n_0)$,  
and so Beta is conjugate to Bernoulli. 


$\theta_{\rm MLE} =$ empirical probability $= \frac{n_1}{n}$  
$\theta_{\rm MAP} = \frac{a+n_1-1}{a+b+n-2}$ 

$\frac{a+n_1}{a+b+n} = \frac{a+b}{a+b+n}\frac{a}{a+b} + \frac{n}{a+b+n}\frac{n_1}{n}$
...
P(X = 1\vert D) & = \int P(X = 1\vert)p(\theta \vert D) \\
                ...
                & = \int \theta {\rm Beta}(\theta \vert a+n_1, b+n_0)d\theta
