...
**Why double networks?**
Deep reinforcement learning 이 tabular reinforcement learning 과 구별되는 중요한 부분 중 하나는 action value 함수의 argument 가 
state, action 에 더해 추가된다는 것이다. Q-learning 의 경우 네트워크를 구성하는 CNN 파라미터 $\theta$ 가 그것이다.
구체적으로 CNN 은 $theta$ 에 대한 다음과 같은 함수를 생성하는 역할을 한다.
