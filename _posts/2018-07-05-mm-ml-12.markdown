...
$p(y\vert x, D) \approx p(y\vert x, D, m^\star)$ where $m^* \in \arg\max_mp(m\vert D)$

$p(m\vert D) \propto p(D\vert m)p(m)$

**Type II ML/Evidence Approximation/Empirical Bayes :**

$p(y\vert x, D) \approx p(y\vert x, D, m^\star)$ where $m^* \in \arg\max_mp(D\vert m)$
