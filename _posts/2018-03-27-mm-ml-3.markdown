...
using the obsrved <span style="color:orange">$p(y\vert x)$</span> as the key quantity. (Decision-theoretic idea)
...
* $p(y\vert x, \theta, x, D)$ : nice in the sense that a closrd form expression can be obtained  
* $p(\theta\vert x, D)$ often times nasty  
* $\int /\sum$ : nasty  
<span style="color:red">As a whole computing $p(y \vert x, D)$ is often intratible.
Many of the techniques in ML come in the course of different ways to solve this problem or parts of this problem of computation.</span>
