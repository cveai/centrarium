...
불확실성이 포함된 상황을 체계적으로 기술하기 위해 수학의 **확률**이란 개념을 이용한다.
확률이라는 개념이 어떻게 탄생하게 되었는지 그 역사적 배경에 관해서는 김민형 교수님의 다음 공개강연 영상의 시청을 권한다: [2].
강연의 중요한 포인트 중 한가지를 언급하자면 ...
...
확률변수 $X$가 의미하는 바를 알아챘는가? 그렇다. $X(•)$ 는 •가 나타내는 주사위 눈의 갯수이다. 이 $X$라는 확률변수를 이용하면 주사위 실험에서 일어나는 사건(event)들을 간편하게 기술할 수 있다. 예를 들면 $[X = 1]$는 확률변수 $X$의 함수값이 1이 되게 하는 표본을 모두 모은 집합이다.
즉, $[X = 1] = \\{ ⚀ \\}$이다. 그러면 $\\{ ⚁, ⚂\\}$는 $X$를 이용하여 어떻게 표현수 있을까?
