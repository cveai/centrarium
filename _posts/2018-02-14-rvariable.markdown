...
확률함수가 주어지면 그에 따른 확률분포를 생각할 수 있음을 앞서 보았다.
아주 간단한 것에서 부터 대단히 복잡한 양상을 띈 것까지 무수히 많은 분포가 존재하 것이다.
확률분포 하나를 숫자하나로 나타낼 수 있을까? 아마 불가능할 것이다.
하지만 하나의 숫자로 확률분포가 가지는 어떤 특성을 대표할 수 있는데 기댓값(expectation)이란 개념이 그 중 하나이다.

$$E[X] = \sum_{a \in \Omega} p(a)a$$ 

여기서 확률함수 $p$와 $X$의 관계는 무엇인가? 좀 복잡할 수 있지만 차분히 살펴보자.
확률함수 $p$가 주어지면 확률분포(혹은 질량 함수) $f_p : \Omega \rightarrow [0, 1]$를 정의할 수 있다:

$$f_p(a) = p(\{a\}).$$

역으로, 질량 함수 $f$가 주어졌을 떄 확률함수 $p_f : \Omega \rightarrow [0, 1]$를 정의할 수 있다.  
이 상호관계를 곰곰히 생각해보는 기회를 가질 것을 권한다.

이제 확률함수 $p$가 주어졌을 때 생성되는 하늘의 별처럼 많은 확률변수들 중에 단하나의 특별한 확률변수 $X$를 정하고 싶다면 과연 어떤 것을 선택해야 할까?
주사위 실험의 예로 돌아가보자. 
