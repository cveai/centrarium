...
Suppose we want to estimate a RV $X \sim p$ where $p: \\{1, \ldots, m\\} \rightarrow [0,1]$ is unknown.

Assume the data $D = (x_1, \ldots, x_n)$ gotten via $X_1, \ldots, X_n \sim p$ iid.

Set the parameter $\theta = (\theta_1, \ldots, \theta_m) = (p_\theta(1), \ldots, p_\theta(m)) \in \Theta=\\{\theta = (\theta_1, \ldots, \theta_m) : \sum_{i=1}^m \theta_i =1\\}$.
...
In other words, we are maximizing the function
$p(D \vert \cdot): \mathbb{R}^m \rightarrow [0,1], \, p(D \vert (\theta_1, \ldots, \theta_m) = \prod_{j=1}^m \theta_j^{nj}$
