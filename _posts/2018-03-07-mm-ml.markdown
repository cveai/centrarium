...
"Natural" estimate of $\theta$: $\delta_1(D) = X \Rightarrow$, bias$^2 = 0$, var$ = 1$, MSE$ =1$  
"Silly" estimate of $\theta$: $\delta_0(D) = X \Rightarrow$, bias$^2 = \theta^2$, var$ = 0$, MSE$ = \theta^2$

cd. Shrinkage, Stein's paradox
