...
want to know from whichi distribution $D$ might come fomm.  
Model $D$ as $X_1, \ldots, x_n \sim N(\mu, \sigma^2)$ for some $\mu, \sigma \in \mathbb{R}$,  
i.e., $N(\mu, \sogma^2)$ is the "true" distribution wa want.  
Becasue we do not know $\mu$, we model it as a random variable $\theta$, which itslef follows a normal distribution:
