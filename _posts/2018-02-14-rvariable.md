...
1. 하나의 확률변수 $X$만 포함된 확률 계산의 경우 기반이 되는 확률함수 $p$를 이용하여 
$$p[X = a]$$

로 표현하는 것이 가장 정확하나 $\mathrm{Pr}[X = a]$이 $\mathrm{pr}[X = a]$, 
$\mathrm{P}[X = a]$, $\mathrm{p}[X = a]$, $\mathrm{Prob}[X = a]$ 등의 표현이 보이면 모두
$p[X = a]$로 해석하면 된다. 이 경우는 확률변수가 하나이므로 큰 혼란은 없다. 
