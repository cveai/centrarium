...
Now it's easy to compute the posterior distribution: 
$p(\theta\vert D) = {\rm Beta}(\theta\vert a+n_1, b+n_0)$, and so Beta is conjugate to Bernoulli. 
...
$\theta \vert D \sim {\rm Beta}(\theta\vert a+n_1, b+n_0)$
