...
<figure>
<img src="/assets/pics/mm-ml/newton.png" alt="Newton's method" style="width: 60%; height: 60%">
<figcaption> Minimizing in 2D
</figcaption>
</figure>

Minimize: 
$0 = \nablaq = Hx + b \Rightarrow x = -H^{-1}b = a -H^{-1}g$

Critical to check: $\nabla^2q = H$ $\Rightarrow$ minimum if $H$ is PSD.
...
3. $x_{t+1} = x_t - \alpha_t y$. (small "step side" $\alpha>0$)
