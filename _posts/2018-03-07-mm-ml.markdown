...
**Probabilistic formulation** (<span style="color:red">Discrimitive model!</span>) Fix $D$ and fix $x$. Fix $k$.

Find a RV $Y \sim p$ where $p(y) =$ #$\\{x_i \in N_k(x) : y_i = y \\}/k$ 

Sometimes people write $p(y\vert y, D)$ for $p(y)$.h

The estimate (or prediction) is giveb by 

$$\hat{y} = \arg\max_y p(y\vert y, D).$$
