...
이 테이블은 사실 다음의 함수를 나타낸 것으로 볼 수 있는데 측도론(measure theory)에 의하면 확률 질량 함수(probability mass function)라 불린다.  
...
어렵지 않게 $X-2Y = Z$가 됨을 확인 할 수 있을 것이다. (사실은 $Z$를 $X - 2Y$의 함수값으로 정의했다.)
