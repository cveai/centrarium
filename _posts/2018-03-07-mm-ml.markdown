...
The process defines a function <span style="color:green">It's not a sampling method but an estimation technique!</span>$y = f(x)$ that is constant on each of the petitioned regions.
